                    ...
                    _loadDataOfWeather.value = loadDataOfWeather.value?.copy(
                        ...
                        forecast = it.forecast.apply {
                            it.forecast.forEach { it1 ->
                                ...
                                it1.backgroundOfItem = if (it.current.isDay != 1) R.drawable.background_for_recycler_items_night else whichDataBackOfItem(it.current.typeOfWeather)
                            }
                        },
                        colorOfRoot = if (it.current.isDay != 1) "#E5E7F7" else "#E8FCFF",
                        colorOfMainElement = if (it.current.isDay != 1) R.drawable.background_main_weather_night else whichDataToSet(it.current.typeOfWeather),
                        resource = Resource.Status.SUCCESS,
                        imageOfMain = if (it.current.isDay != 1) R.drawable.ic_clear else whichDataImage(it.current.typeOfWeather)
                    )
                },
                onFailure = {
                    _loadDataOfWeather.value = loadDataOfWeather.value?.copy(
